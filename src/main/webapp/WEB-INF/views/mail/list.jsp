...
$(function () {
    ...
    function updateMailCount() {
        $.ajax({
            ...
            success: function(json) {
                $('span#count').html(json.count + " / " + json.totalCount);
            },
            error: function() { console.error("메일 카운트 가져오기 실패"); }
        });
    }
    updateMailCount();
    ...
    // 기타 클릭 이벤트 방지
    $(document).on("click", ".attach-icon, input[name='chkMail']", function(event){ event.stopPropagation(); });

    // 전체 체크박스
    $(document).on("change", "#chkAll", function() { $("input[name='chkMail']").prop("checked", this.checked); });
    $(document).on("change", "input[name='chkMail']", function() {
        $("#chkAll").prop("checked", $("input[name='chkMail']:checked").length === $("input[name='chkMail']").length);
    });

    // 삭제 버튼
    $(document).on("click", "#btnDelete", function() {
        let checkedMails = $("input[name='chkMail']:checked");
        if (checkedMails.length === 0) { alert("삭제할 메일을 선택하세요."); return; }
        if (!confirm("선택한 메일을 삭제하시겠습니까?")) return;

        let emailSeqArr = [];
        checkedMails.each(function() { emailSeqArr.push($(this).val()); });
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function(json) { if (json.n > 0) { alert("메일이 삭제되었습니다."); location.reload(); } }
        });
    });
...
<main style="margin-left: 240px; padding-top: 20px;">
    <div class="container-fluid">
        ...
        <div class="card mb-4 shadow-sm">
            <div class="card-body">
                <form id="searchFrm" method="get" action="<%=ctxPath%>/mail/list" class="row g-2 align-items-center">
                    ...
                    <div class="col-md-2 col-sm-12">
                        <select id="filter" name="filter" class="form-select" onchange="this.form.submit()">
                            <option value="all" ${param.filter == 'all' ? 'selected' : ''}>전체 메일</option>
                            <option value="unread" ${param.filter == 'unread' ? 'selected' : ''}>안 읽은 메일</option>
                            <option value="important" ${param.filter == 'important' ? 'selected' : ''}>중요 메일</option>
                            <option value="attach" ${param.filter == 'attach' ? 'selected' : ''}>첨부 메일</option>
                        </select>
                    </div>
